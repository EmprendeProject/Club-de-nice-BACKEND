...
def delete_chapter(course_id: str, chapter_id: str) -> dict:
    """
    Raises:
        HTTPException 404 — capítulo no encontrado
        HTTPException 500
    """
    ...
    try:
        ...
        msg = supabase_error(exc)
        logger.error("[courses.delete_chapter] lookup FAILED chapter_id=%s [%s] %s", chapter_id, type(exc).__name__, msg, exc_info=True)
